...
public void init(){
   ...
   public void CategoryService_Save_ReturnSavedCategory() {
    ...
    void CategoryService_addArticleToCategory_CategoryFound_ArticleFound() {
        ...
        when(articleRepository.findById(articleId)).thenReturn(Optional.of(recipe));
        when(categoryMapper.map(any(Optional.class), Mockito.eq(Recipe.class))).thenReturn(recipe);
